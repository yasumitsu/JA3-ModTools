...
function Unit:HasSignatures()
	for _, perk in ipairs(self:GetPerks()) do
		if perk.Tier == "Personal" then
			if (perk.class == "DoubleToss") and
				(((self.ui_actions.DoubleTossA or "hidden") ~= "hidden") or
				((self.ui_actions.DoubleTossB or "hidden") ~= "hidden") or
				((self.ui_actions.DoubleTossC or "hidden") ~= "hidden") or
				((self.ui_actions.DoubleTossD or "hidden") ~= "hidden"))
			then
				return true
			else
				if (self.ui_actions[perk.class] or "hidden") ~= "hidden" then
					return true
				end
			end
		end
