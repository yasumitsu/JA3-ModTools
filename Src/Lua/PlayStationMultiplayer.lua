...
local s_WaitNotifyThread = false

function OnMsg.NetGameLoaded()
	s_WaitNotifyThread = CreateRealTimeThread(function()
		WaitGameState({ loading = false, gameplay = true, setpiece_playing = false, entering_sector = false, entered_sector = true })
		...
		s_WaitNotifyThread = false
	end)
end

function OnMsg.NetGameLeft()
	if IsValidThread(s_WaitNotifyThread) then 
		DeleteThread(s_WaitNotifyThread)
		s_WaitNotifyThread = false
	end
end
