...
function ModItem:GenTTable()
	local tTables = {}
	self:ForEachSubObject("PropertyObject", function(obj, parents)
			...
			if propVal ~= "" and IsT(propVal) then
				...
				if getmetatable(propVal) == TConcatMeta then
					for _, t in ipairs(propVal) do
						tTables[#tTables+1] = { id = TGetID(t), text = TDevModeGetEnglishText(t), context = context, voice = voice }
					end
				else
					tTables[#tTables+1] = { id = TGetID(propVal), text = TDevModeGetEnglishText(propVal), context = context, voice = voice }
				end
			end
		end
	end)
