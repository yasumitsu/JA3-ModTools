...
function ModItemMapPatch:GenTTable()
	local tTables = ModItem.GenTTable(self)
	if self.MapPatchPath then
		local func, err = loadfile(self.MapPatchPath)
		if not err then
			for _, op in ipairs(table.get(func(), "objects", "data")) do
				for k,v in pairs(op.after) do
					if IsT(v) then
						tTables[#tTables+1] = { id = TGetID(v), text = TDevModeGetEnglishText(v), context = string.format("map patch %s %s", self.Map or "", op.after and op.after.class or "") }
					end
				end
			end
		end
