...
AppendClass.GameRuleDef = {	
	properties = {
		...
		{ id = "show_in_new_game", name = "Show in New Game", category = "General",help = "Show the game rule in New Game menu", editor = "bool", default = true}
	}
}
...
function CloseOptionsChoiceSubmenu(ui)
	local dlg = GetDialog(ui)
	local dialog = dlg and dlg:ResolveId("idSubSubContent")
	...
	if dialog then
		...
		local sub_menu = dlg and dlg:ResolveId("idSubMenu")
		local scroll_area = sub_menu and sub_menu:ResolveId("idScrollArea")
		if  scroll_area then
			scroll_area:SetMouseScroll(true)
		end
	end
end
