...
if not Platform.cmdline and Platform.pc and Platform.developer then
	...
	function RegisterSavFileHandler()
		...
		local name = "hg-" .. const.project
		local reg = string.format([=[reg add HKCU\Software\Classes\%s\shell\open\command /f /d "cmd /c start \"%s\" /d \"%s\" \"%s\" -save \"%%1\""]=], name, const.project, GetCWD(), path)
