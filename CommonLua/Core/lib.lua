...
function ConvertToFileServerProjectPath(path)
	path = string.gsub(path or "", "/", "\\")
	if string.starts_with(path, "\\") then
		path = string.sub(path, 2)
	end
	local fspp = config.FileServerProjectPath or ("\\\\bender.haemimontgames.com\\" .. (const.project or ""))
	return fspp .. "\\" .. path
end

ConvertToBenderProjectPath = ConvertToFileServerProjectPath -- deprecated, please don't use
