---@diagnostic disable: duplicate-set-field, param-type-mismatch, need-check-nil
function ResetColorModifier(parentEditor, object, property, ...)
	object:SetProperty(property, const.clrNoModifier)
end

function GetCollectionNames(CollectionsByName)
    if CollectionsByName then
...
-- Remove the duplicate definition here
-- function CObject:GetEnumFlagsProp()
-- 	return self:GetEnumFlags()
-- end
...
function CObject:OnEditorSetProperty(prop_id, old_value, ged, multi)
	...
	if editor.IsModdingEditor() then
		-- for users modding maps:
		  -- if they are modifying translatable properties which are already translated (from the base game, or from their own localization texts)
		  -- reset the translation ID, so the new string appears untranslated, instead of fetching the old translation
		local new_value = self[prop_id]
		if IsT(new_value) and IsT(old_value) then
			local old_id = TGetID(old_value)
			local new_id = TGetID(new_value)
			if old_id and new_id and old_id == new_id and TranslationTable[old_id] and old_value[2] ~= new_value[2] then
				self[prop_id] = DuplicateT(new_value)
			end
		end
	end
end
