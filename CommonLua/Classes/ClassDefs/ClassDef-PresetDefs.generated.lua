---@diagnostic disable: duplicate-set-field, unbalanced-assignments, undefined-field
...
function AnimMetadata:OnPreSave()
    for _, moment in ipairs(self.Moments) do
        moment.AnimRevision = moment.AnimRevision or 0 -- initialize with a default value
...
function GameRuleDef:IsCompatible(active_rules)
    local exclusions
    if self.exclusionlist then
        exclusions = table.invert(self.exclusionlist)
    else
        exclusions = {}
    end
    if exclusions then -- Add this check
        local id = self.id
...
function LootDef:GenerateLootSeed(init_seed, looter, looted)
	...
	if loot == "cycle" or loot == "each then last" then
		seed = (init_seed == -1) and InteractionRand(nil, "Loot", looter, looted) or (init_seed + 1) or 0
...
DefineClass.LootDefEntry = {
	...
	StoreAsTable = true,
	EditorView = Untranslated("<if(disable)><color 192 0 0>off</color> <color 128 128 128> | </color> </if><FormatAsFloat(weight,1000)><opt(u(dlc), '<color 128 128 128> | </color>', '')><if(negate)>!</if><opt(u(condition), '<color 128 128 128> | </color>', '')><color 128 128 128> | </color><EntryView><color 0 128 0><opt(u(comment),'<color 128 128 128> | </color>','')>"),
...
DefineClass.LootEntryLootDef = {
	...
	EntryView = Untranslated("Loot def:<color 75 105 198><loot_def> <if(not_eq(amount_modifier,1000000))>x<FormatAsFloat(amount_modifier,1000000,3,true)></if>"),
